...
class GlfwWindow(width: Int, height: Int, title: String) : Window {
    ...
    override var framebufferWidth: Int = width
        private set
    override var framebufferHeight: Int = height
        private set
    ...
    init {
        ...
        glfwSetKeyCallback(handle) { _, key, _, action, _ ->
            val glfwInput = Input.delegate as? GlfwInput
            if (glfwInput != null) {
                when (action) {
                    GLFW_PRESS   -> glfwInput.setKey(key, true)
                    GLFW_RELEASE -> glfwInput.setKey(key, false)
                }
            }
        }
        glfwSetCursorPosCallback(handle) { _, xpos, ypos ->
            (Input.delegate as? GlfwInput)?.setMousePosition(xpos.toFloat(), ypos.toFloat())
        }
        glfwSetMouseButtonCallback(handle) { _, button, action, _ ->
            (Input.delegate as? GlfwInput)?.setMouseButton(button, action == GLFW_PRESS)
        }
